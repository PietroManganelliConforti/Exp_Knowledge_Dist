...
class _CAM:
    ...
    def _hook_a(self, _: nn.Module, _input: Tuple[Tensor, ...], output: Tensor, idx: int = 0) -> None:
        ...
        if self._hooks_enabled:
            #print("hook_a")
            #print(output.shape, output.requires_grad, output.device)
            #print(idx)
            #print(self.target_names)
            #print(output.data.shape, output.data.requires_grad, output.data.device)
    ...
    def _normalize(cams: Tensor, spatial_dims: Optional[int] = None, eps: float = 1e-8) -> Tensor:
        """CAM normalization."""
        #print("normalize")
        #print(cams.shape, cams.requires_grad, cams.device)

        spatial_dims = cams.ndim - 1 if spatial_dims is None else spatial_dims
        cams = cams - cams.flatten(start_dim=-spatial_dims).min(-1).values[(...,) + (None,) * spatial_dims]
        # Avoid division by zero
        cams = cams / (cams.flatten(start_dim=-spatial_dims).max(-1).values[(...,) + (None,) * spatial_dims] + eps)
    ...
    def compute_cams(
        ...
        **kwargs: Any,
        ...
        #print("weights[0]", weights[0].shape, weights[0].requires_grad, weights[0].device)

        cams: List[Tensor] = []

        #print("self.hook_a", self.hook_a[0].shape, self.hook_a[0].requires_grad, self.hook_a[0].device)
        #print("len self.hook_a", len(self.hook_a))


        #self.hook_a[0].requires_grad_(True)
        ...
        for weight, activation in zip(weights, self.hook_a):
            #print("weight", weight.shape, weight.requires_grad, weight.device)
            #print("activation", activation.shape, activation.requires_grad, activation.device)
            ...
            #print("cam", cam.shape, cam.requires_grad, cam.device)

            if self._relu:
                cam = F.relu(cam)  # Modified to be non-in-place
