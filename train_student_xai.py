...
from helper.loops import train_distill as train, validate_vanilla, validate_distill, train_distill_xai
...
def parse_option():
    ...
    # XAI
    parser.add_argument('--xai', default='gradcam', type=str, choices=['gradcam', 'scorecam', 'fullcam', 'gradcamplspls', 'xgradcam', 
                                                                       'eigencam', 'eigengradcam', 'layercam', 'fullgrad', 'hirescam', 'gradcamelementwise'])
...
def main_worker(gpu, ngpus_per_node, opt):
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        train_acc, train_acc_top5, l_base, l_kd, l_xai = train_distill_xai(epoch, train_loader, module_list, criterion_list, optimizer, opt)
        time2 = time.time()
...
if __name__ == '__main__':
    print(torch.cuda.is_available()) # CHECK IMMAGINE DOCKER PIETRO
    main()
